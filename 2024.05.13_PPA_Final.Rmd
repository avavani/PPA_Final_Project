...
library(pacman)
p_load(dplyr, ggplot2, , patchwork, sf, stats, tidycensus, tidyr, mapview, tigris, tmap)
```
...
```{r bmore_clean_data, message=FALSE, warning=FALSE}
#percentage of vacant lots 
...
#the geographies are all over the place, will have to clean - maybe areal interpolation? 
#based on the joins, a lot of the CSAs encompass multiple tracts, which makes sense since there's 199 tracts and only 55 CSAs

```

# Defining Gentrification
To create a Gentrification Index, we calculate the proportion of low and high income households for each census tract in 2022 and 2012. We subtract the change in low income proportion of households from the high income proportion over the ten years.
...
```{r bmore_data2, message=FALSE, warning=FALSE}
...
```{r bmore_clean_data2, message=FALSE, warning=FALSE}
#percentage of vacant lots 
bmore_p_lots <- bmore_p_lots %>%
  select(CSA2020, vacant21, geometry) %>%
  rename(CSA = CSA2020, 
...
bmore_school <- school_outcome %>%
  st_centroid()

bmore_CSA_data <- st_join(bmore_CSA_data, bmore_school, left = TRUE)

bmore_walk <- walk_score %>%
  st_centroid()

bmore_CSA_data <- st_join(bmore_CSA_data, bmore_walk, left = TRUE)
