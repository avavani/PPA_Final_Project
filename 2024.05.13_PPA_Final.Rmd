...
```{r finalacs, include=FALSE, warning=FALSE, message=FALSE}
all.acs <- bmore_acs17 %>%
...
At lower percentiles, the model is more likely to classify tracts as gentrifying, which increases the TPR but also leads to a higher FPR. As the percentile increases, the TPR decreases while the FPR also decreases, meaning the model becomes more conservative in predicting gentrification, thus reducing the number of false positives but also missing some true positives.
...
```{r MAE_map, warning=FALSE, message=FALSE}
ggplot() +
  geom_sf(data = water_bound, fill = "steelblue") +
  geom_sf(data = md_bound, fill = "gray80", col = "gray75") +
...
"The Black Butterfly": Racial Segregation and Investment Patterns in Baltimore (https://apps.urban.org/features/baltimore-investment-flows/)
